# override-evaluation Specification
...
The script SHALL confine any side effects required to compute the natural resolution
(file copies, scratch installs, temporary `node_modules`, temporary lockfiles) to a
location outside the consumer's working directory. The consumer's `node_modules`, `package-lock.json`,
...
The script SHALL classify an override as `removed` if it targets a package that is
not present anywhere in the consumer's natural dependency graph (i.e., no dependent
declares it directly or transitively), since such an override is dead weight.
