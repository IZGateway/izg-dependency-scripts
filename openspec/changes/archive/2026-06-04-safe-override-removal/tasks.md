...
- [x] 7.2 Open a PR with label `bump:minor` so `ci.yml` cuts the next minor on merge — *PR #6 (`IGDD-2967 - override removal check`) merged to `main` 2026-06-04 with no label, so `ci.yml` defaulted to a patch bump. Released as `v1.0.5` instead of a minor. Accepted as-is — the code is shipped and consumable; the patch-vs-minor label is cosmetic.*
- [x] 7.3 After merge, verify `@izgateway/dependency-scripts@1.1.0` (or whichever minor is cut) lands on the registry and the floating `@v1` tag advances — *Released as `v1.0.5` (not 1.1.0, see 7.2). `@v1` floating tag advanced to point at the `v1.0.5` release commit `4648a5a`.*
- [x] 7.4 After release, follow up with `IZGateway/izg-configuration-console`: re-add `"postcss": "8.5.15"` and `"ajv": "8.20.0"` to `overrides` if PR #521 has merged, and confirm the next nightly run reports them as `kept` — *Verified manually against `console@develop` with `@izgateway/dependency-scripts@1.0.5` (`npm install --no-save @1.0.5`, then `test-overrides.js`). Both overrides already present on `develop` (no re-add needed). Result: `postcss` correctly `kept` (next's postcss would drop to 8.4.31, below floor); `ajv` correctly `removed` (node_modules/ajv resolves to 8.20.0 regardless). Stale `fast-uri`/`http-proxy-agent` also removed; nested/alias forms skipped; exit code 2 (skips present). NOTE: the nightly's gated "Test override removal" step never runs on its own (gated behind has_changes) and runs `test-overrides.js` under `bash -e`, so exit 2 would fail the step — tracked as a separate `izg-configuration-console` workflow fix, not part of this change.*